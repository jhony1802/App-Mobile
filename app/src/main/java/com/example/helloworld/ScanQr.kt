...
import me.dm7.barcodescanner.zxing.ZXingScannerView
 ...
 class ScanQr : AppCompatActivity(), ZXingScannerView.ResultHandler {
    companion object {
        ...
        fun getScanQrCodeActivity(callingClassContext: Context) = Intent(callingClassContext, ScanQr::class.java)
    }
    override fun onCreate(savedInstanceState: Bundle?) {
     ...
     override fun handleResult(p0: Result?) {
         if (p0 != null) {
             startActivity(ScannedQr.getScannedActivity(this, p0.text))
             resumeCamera()
         }
     }
     /**
     ...
     private fun resumeCamera() {
         Toast.LENGTH_LONG
         val handler = Handler()
         handler.postDelayed({ qrCodeScanner.resumeCameraPreview(this@ScanQr) }, 2000)
     }
}